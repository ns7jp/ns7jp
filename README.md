...
> このポートフォリオは、**実行した記録と、まだ実行していない範囲を分けて書いています。**
> 言葉の定義・前提・未実施の一覧は [この資料の読み方](#この資料の読み方) にまとめました。
> 各表の「限界」列と合わせてご確認ください。
...
主な未実施範囲は、監視ラボ全体を独立した引き渡し対象ホストへ構築すること、組織 DNS を含む本番相当のネットワーク確認、Slack への実配信、AWS への実適用、ホスト再起動後の永続性・長期稼働、負荷試験、D-2 以降の障害復旧演習です。AlmaLinux は基礎設定までで、監視ラボ全体の `site.yml` 適用は未実施です。
