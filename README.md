...
インフラはまだ初心者で、設計の理由を細かく説明できない部分も多くあります。ここに挙げるのは「実際に手を動かした範囲」です。

- Ubuntu のユーザー、sudo、SSH、UFW、更新、timezone の初期設定
- Docker Compose を使った Nginx / Flask / 監視スタックの配備
- Ansible で役割ごとに role を分けて構成管理、Vault で秘密値を管理
...
文書の構成・整形・調査、コードレビュー、リンクや表記の確認には AI 支援を広く使っています（`claude/*` のブランチ名がその作業単位です）。

一方、[LEARNINGS.md](./LEARNINGS.md) の各エントリと [server-monitor の証跡ファイル](https://github.com/ns7jp/server-monitor/tree/main/docs/evidence)は、すべて自分が実機で詰まって手を動かした一次記録です。技術選定の最終判断、実機操作、結果の採録、機密情報のマスク、面接での説明は本人が担当します。AI が生成した手順も、本人が実行・理解していない状態では実績として扱いません。面接では、この一次記録についてコマンドと出力のレベルでご質問いただけます。
