...
flowchart LR
    ...
    subgraph Next[改善計画 v1.1 - v2.0]
        ...
        N3[+ Ansible 構成管理]
        N4[+ SLO / バーンレート]
        N5[+ インシデント運用<br/>+ ポストモーテム]
        N6[+ 復旧演習]
        N7[+ セキュリティ運用]
        N8[+ AWS / Terraform<br/>2 AZ 冗長化]
    end
...
詳細：[アーキテクチャ図（現状 / 将来構想）](./docs/architecture-diagram.md) ／ [ビジュアルショーケース](./docs/showcase/README.md)
...
server-monitor を実運用水準へ引き上げるため、本リポジトリ内に **改善設計書を 9 本** 整備しました。
