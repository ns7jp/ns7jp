...
具体的なOS導入、固定IP、SSH、初期更新は[既存のPhase 1演習設計](../../../../docs/learning-plan/05-phase1-exercise-design.md)を正本として使います。準備段階で同じ工程を実行し、初期状態からの証跡を残していれば、その自分の証跡を参照できます。OS導入済みイメージを起動しただけなら、空VMへの導入工程は別途必要です。
...
5. [既存演習の3-7〜3-9](../../../../docs/learning-plan/05-phase1-exercise-design.md)に従って鍵接続と到達制御を構築します。**鍵で別セッションから接続できることを確かめてから**パスワード認証の制限へ進みます。コンソールと既存の接続を維持します。
...
SSHは管理経路なので、ここで停止故障の題材にはしません。SSHの設定変更が必要なら既存Phase 1の順番と`sshd -t`による確認を使い、接続を残した状態で行います。更新は[既存Phase 1の3-10](../../../../docs/learning-plan/05-phase1-exercise-design.md)で、対象VM、更新前後、再起動要否、復旧方法を記録して実施します。`apt list --upgradable`だけでは更新済みになりません。
...
基礎コマンドが難しければ[W1〜W4のカリキュラム](../../../../docs/learning-plan/02-curriculum.md)と[Phase 1演習設計](../../../../docs/learning-plan/05-phase1-exercise-design.md)へ戻り、1日1観点に分けます。失敗は消さず再試行を追加し、評価を受けてから[SE02](./02-network.md)へ進みます。
