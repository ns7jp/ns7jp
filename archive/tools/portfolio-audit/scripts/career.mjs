...
// 行動計画の reference には archive/docs/ に移した文書と、リポジトリ直下の docs/ に
// 残した文書の両方が現れるため、archive 側 -> リポジトリ直下 の順で探す。
// safe() の検査は各 root に対してそのまま適用するので、外へ抜ける経路は増えない。
function readDoc(root, relative) {
  try { return read(root, relative); }
  catch (error) {
    const parent = path.resolve(root, '..');
    if (parent === path.resolve(root)) throw error;
    return read(parent, relative);
  }
}
...
export function validate(input, now, root) {
  ...
  for (const a of input.actions) {
    ...
    check(a.reference.startsWith('docs/'), 'Action must refer to an existing document'); readDoc(root, a.reference);
