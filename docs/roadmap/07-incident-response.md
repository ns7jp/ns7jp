...
> **組織前提の扱い**
>
> Sev 定義・ロール分担・エスカレーション基準など、複数人チームを前提とした記述は
> **組織適用時の設計サンプル** である。個人ラボでは §3.1「個人ラボでの読み替え」の
> 軽量版で運用する。
...
> 学習メモ：障害「対応の技術」だけでなく、**「組織として障害から学ぶ仕組み」** も運用の一部だと理解している。個人ラボでもその型だけは真似しておきたい。
...
| 組織前提の記述 | 個人ラボでの運用 |
| --- | --- |
| 30 分以内に上長へ報告（§2 Sev1） | 自分宛 Slack 通知 + インシデント Issue へのタイムライン記録 |
| 翌営業日朝に報告（§2 Sev2） | 翌日、Issue に状況サマリをコメント |
| IC / Investigator / Comms / Scribe の分担（§3） | 1 人で帽子を掛け替える運用（前掲「1 人体制での運用」の通り） |
| Slack 専用チャンネルでの宣言・周知（§5） | 自分の Slack ワークスペースに `#inc-YYYY-NNNN` を作成し、同じテンプレで記録 |
| 月次レビュー会議体（§8） | 月初に 1 人レビューを実施し、結果を Issue に記録して証跡化 |
