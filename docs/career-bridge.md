...
本ラボの主要クラウド／IaC 系統は AWS + Terraform（[ADR-0005](./adr/0005-terraform-for-iac.md)、[03 AWS + Terraform](./server-monitor-improvements/03-terraform-aws.md)）ですが、トライアル就業（2026-09-15 に終了）の研修には Windows Server / AD / Linux / AWS / **Azure** の構築が含まれていました（[志望の経緯](#志望の経緯)）。国内 SIer・大手企業の社内基盤では Microsoft 365 / Entra ID との親和性から Azure の採用例も多いと理解しています。クラウドが違っても、アカウント境界・ネットワーク・IAM・IaC・監視・バックアップという設計の骨格は対応するため、次の対応で読み替えて習得します。
...
人材派遣会社（アデコ株式会社）を通じた IT 企業でのトライアル就業（2026/07〜）で、Windows Server / Linux サーバーおよび AWS / Azure の構築研修に取り組みました。トライアル就業は 2026-09-15 に終了し、**現在は求職中で、すぐに勤務を開始できます**。就業先の社名は面談時に開示します。

研修は仮想マシン上の演習環境が中心で、引き渡しや受け入れ試験、実障害対応までを一貫して担当したわけではありません。目指しているのは、サーバー構築エンジニアとして直接雇用（正社員）の立場で技術を積み上げることです。
