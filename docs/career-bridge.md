...
| 物流現場 | インフラ運用 | 該当ドキュメント |
| --- | --- | --- |
| 場所マップで「新人でも探せる」 | 構成図 + ランブックで「当番でも復旧できる」 | [server 構成](https://github.com/ns7jp/server/blob/main/docs/architecture.md) |
...
| 物流現場 | インフラ運用 | 該当ドキュメント |
| --- | --- | --- |
| 機械停止時の手作業フォロー | プロセス障害時のフェイルオーバー | [server ランブック](https://github.com/ns7jp/server/blob/main/docs/runbooks/service-down.md) |
...
本ラボの主要クラウド／IaC 系統は AWS + Terraform（[ADR-0005](./adr/0005-terraform-for-iac.md)、[03 AWS + Terraform](./server-monitor-improvements/03-terraform-aws.md)）ですが、2026-09-11 に確認した派遣先の研修には Windows Server / AD / Linux / AWS / **Azure** の構築が含まれます（[志望の経緯](#志望の経緯)）。現在の終了・継続状況は未確認です。国内 SIer・大手企業の社内基盤では Microsoft 365 / Entra ID との親和性から Azure の採用例も多いと理解しています。クラウドが違っても、アカウント境界・ネットワーク・IAM・IaC・監視・バックアップという設計の骨格は対応するため、次の対応で読み替えて習得します。
...
ここまでの転用マップは、計測・標準化・セキュリティ統制という**概念レベル**の橋渡しが中心でした。ここでは、物流現場で日常的に行っていた**体を動かす現物スキル**が、データセンター現地作業にそのまま使える対応を挙げます。

| 物流現場 | データセンター現地作業 |
| --- | --- |
| フォークリフト・台車での重量物運搬 | サーバー・ラック機材の搬入、ラックマウント |
| ハンディ端末でのバーコード資産管理・棚卸し | 資産管理台帳（ラベル / バーコード）によるハードウェア棚卸し |
| 結線・梱包の整理（誤配送防止） | ケーブルマネジメント、ラベリング規則 |
| 入出庫時のダブルチェック | 入退室管理、作業記録（メンテナンス作業のログ） |

また、現場作業からの転職では「長時間の PC 作業・タイピング・複数モニターでのオペレーション・チケット管理ツールの操作」といったデスクワークへの適応も見られやすいポイントです。学習・ポートフォリオ制作自体が日々の Linux ターミナル操作と Markdown でのドキュメント作成を伴うため、机上作業への適応はこの制作プロセスを通じて進めています。

**この節はまだ概念の橋渡し段階で、データセンター現地作業そのものの実務・実習経験はありません。** 実際の現地作業（ラックマウント演習等）を行った場合は、[LEARNINGS.md](../LEARNINGS.md)と証跡台帳に記録します。
...
| 業務 | 私が貢献できること | 根拠 |
| --- | --- | --- |
| サーバー監視・運用 | 構築 → 監視 → 障害対応のサイクル設計と実装 | [server](https://github.com/ns7jp/server) |
...
### 1. 就業先の研修で何をやり、何が足りなかったか

就業状況の最終確認日は 2026-09-11 です。同日時点では、人材派遣会社（アデコ株式会社）を通じたトライアル就業（2026/07〜）で、Windows Server / Linux サーバーおよび AWS / Azure の構築研修に取り組んでいました。9 月 15 日は当時の終了予定であり、**2026-09-17 現在の終了・継続と入社可能日は本人確認待ち**です。

確認済みの研修は仮想マシン上の演習環境が中心で、引き渡しや受け入れ試験、実障害対応までを一貫して担当した実績とは区別します。目指しているのは、サーバー構築エンジニアとして直接雇用（正社員）の立場で技術を積み上げることです。入社可能日は現在の状況を確認してから応募先と個別に相談します。

### 2. なぜネットワークでもクラウドでもアプリでもなく、サーバー構築か

「計測 → 仮説 → 実施 → 検証 → 標準化」のサイクルを IT で再現したいという動機は、
ネットワークでもクラウドでもアプリでも成り立ちます。その中でサーバー構築を選んでいるのは、
...
側の DNS が外部を向いていたためドメインに参加できなかった事象です。Linux で慣れていた `dig` による
切り分けと同じ考え方が、Windows 側でも通用することを確認しながら原因にたどり着きました。どちらも
共通しているのは、表面的な症状（冪等性が壊れる／ドメインに参加できない）だけでなく、設定同士の
矛盾や前提条件の見落としという、構築の土台部分に原因があった点です。この「土台を正しく作る」作業
そのものに一番の手応えを感じており、これがサーバー構築を志望する理由です。

### 3. 監視・運用を「入口」として受ける位置づけ

第一志望はサーバー設計・構築ですが、未経験者が最初から構築案件を任されることは少なく、
監視・運用は本番環境の挙動や障害パターンを実地で理解できる、現実的な入口だと考えています。特に
24 時間 365 日の監視業務は、実際に稼働しているシステムがどう壊れ、どう検知され、どう復旧するかを
体感できる機会であり、その経験は「壊れにくい構築」を意識できるエンジニアになるために欠かせないと
考えています。監視・運用を最終的な目的地ではなく、構築の質を上げるための土台として位置づけている、
というのが「結局どちらを志望しているのか」という問いへの答えです。
