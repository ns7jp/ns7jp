...
2026-09-11 時点では、人材派遣会社（アデコ株式会社）を通じたトライアル就業（2026/07〜、現在のトライアル就業は **2026-09-15 終了予定**）
という形態で、Windows Server / Linux サーバーおよび AWS / Azure の構築研修に取り組んでいます。
研修は仮想マシン上の演習環境が中心で、引き渡しや受け入れ試験、実障害対応までを一貫して担当する
機会はありません。ただし、研修の内容そのものに不満があるわけではなく、一番の理由は雇用形態です。
現在は派遣によるトライアル就業であり、目指しているのはサーバー構築エンジニアとして直接雇用
（正社員）の立場で腰を据えて技術を積み上げていくことです。トライアル就業の終了を見据え、
直接雇用のポジションへ移ることを志望しています。入社可能日は応募先と個別に相談します。
...
です。ログを読み込み、
`ufw` モジュール自体ではなくロール側の設計に原因があると切り分けて修正しました。2 件目は、現在の
