...
Zabbix 側については、上記の対応表を実機で検証する演習設計を[08 Zabbix 監視基盤構築演習設計](./learning-plan/08-zabbix-monitoring-exercise-design.md)にまとめています（設計のみ・未実施）。JP1 系は自宅ラボでの再現が難しいため、対応表による概念の橋渡しに留めています。
