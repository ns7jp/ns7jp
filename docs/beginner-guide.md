...
今日は **「利用者 → 入口 → アプリ」の流れを、図を見ずに説明する** ところまで進めます。PCの設定変更やインストールは必要ありません。10分は読む・話す時間の目安です。

1. [1章](#1-この作品は何を作っているのか)で、このラボの目的を一文で言う。
2. [3章のA](#a-人がアプリを見る経路)で、要求と応答の矢印を指で追う。
3. Aの末尾の3問に答え、分からない言葉を一つだけ読み直す。

最初に使う言葉は、**クライアント＝要求する側、サーバー＝応答する側、Nginx＝通信の入口、app＝応答を作るアプリ**の四つです。2章の環境用語と、3章B・Cの監視製品は必要になったときに読みます。

| いまの状態 | 次に行うこと | 今日はここまででよい |
| --- | --- | --- |
| 初めて読む | 上の1〜3を行う | 要求と応答の往復を言える |
| 流れは分かるが、Linux環境がない | [開始前診断](./learning-plan/00-start-here.md)で準備する環境を決める | 準備できた点と不足を記録する |
| 専用のUbuntu環境を用意できた | [主作品の初回実習](https://github.com/ns7jp/server/blob/main/docs/beginner-learning-guide.md)で前提確認から進める | 一つの確認について予想・出力・判定を残す |
| 自分の確認結果がある | [30秒の説明練習](./portfolio-explanation.md#1-まず-30-秒で作品を紹介する)へ進む | 自分の操作と未確認の範囲を言える |

ここから先は、必要な箇所を調べるための説明です。
...
**読み終えたら図を隠して、次の3問に答えます。** 答え合わせは直後の段落で行います。

1. 要求を出すのは誰で、どこを通ってアプリへ届きますか。
2. アプリの応答は、どこを通って利用者へ戻りますか。
3. VMでアプリを起動しました。Windowsの `127.0.0.1` は、そのVMを指しますか。

解答の要点：①ブラウザーがNginxを通じて要求する。②アプリからNginxを通じてブラウザーへ戻る。③Windows自身を指すので、VMを指さない。語句の丸暗記より、誰が要求して誰が返すかを説明できることを確かめます。

**初日はここで区切れます。** 次は[初回実習の前提確認](https://github.com/ns7jp/server/blob/main/docs/beginner-learning-guide.md)へ。B・Cは、2サービスの応答確認ができてから監視を追加するときに読みます。
