...
> **本ドキュメントの位置付け**
>
> ポートフォリオで採用している主要な技術選定について、**「何を選んだか」「他に何を見たか」「選んだ理由」** を簡単に記録します。
> 公式ドキュメントや記事を調べながら書いた学習記録であり、実務での意思決定の経験に基づくものではありません。
> インフラはまだ初心者のため、深いトレードオフ比較までは書いていません。「なぜその選択肢を切り捨てたか」よりも
> 「調べた範囲で何を選んだか」を残すことを優先しています。
...
- 新規 ADR は 4 桁連番でファイル名を付与（例：`0009-xxx.md`）。番号は決定日の前後関係ではなく、ADR を書いた（リポジトリに追加した）順序を表す
...
製造・物流の現場で 15 年以上働いた経験から、**「決めた人がいなくなると、なぜそうなっているか誰も説明できない」** 状況の怖さを学びました。
