...
- 認証失敗は Loki に集約し、しきい値でアラートする（[今後の興味リスト（セキュリティ運用）](../roadmap/README.md)）
...
- アカウントの作成・変更・削除のやり方は v1.0（手動）と v2.0（IdP 側で一元管理）で変わる予定。詳細は[今後の興味リスト（ID 運用）](../roadmap/README.md)
...
- **見直し内容**：v2.0 の OIDC 移行先は **Keycloak / Authentik（自己ホスト・無料）+ oauth2-proxy** を第一候補にする。Grafana は Generic OAuth で Keycloak と統合し（[今後の興味リスト](../roadmap/README.md)）、Prometheus / Alertmanager は oauth2-proxy 経由にする
- **IAM Identity Center の位置付け**：AWS アカウント自体（Console / CLI）の管理には引き続き使う想定だが、カスタムアプリとの連携は選択肢の一つに降格した（[今後の興味リスト](../roadmap/README.md)）
