...
**Linux サーバーと Windows サーバーで人手でやっている日々の点検・バックアップ・後片付けを、Python のプログラム 3 本に置き換え、OS 標準の定時実行の仕組みに載せるところまでを設計した演習です。**

作るものは次の 3 本です。3 本はコードを共有しない独立したツールで、置き場所も実行アカウントも別々です（[2 章 基本設計](#基本設計構成)）。

| ツール | 何をするか | 例えるなら |
| --- | --- | --- |
| `routine.py` | ディスクの使用率・サービスの稼働・ログの直近エラーを毎日点検し、古い一時ファイルを片付ける。既定では「消す予定の一覧」をログに出すだけで、実際に消すのは、人がその一覧を確認してから `--apply` を付けて手動で実行したときだけ | 毎朝の見回り |
| `backup.py` | 設定ファイルを毎日圧縮して保存し、7 世代より古いものを捨てる。保存したアーカイブはハッシュ（SHA-256）と突き合わせて壊れていないかを確認し、`restore` を実行したときは別の場所へ展開して元データと中身が一致するかまで確かめる | 写しの保管と、戻せるかの点検 |
| `check.py` | CPU・メモリ・ディスクの使用率、Web の応答（HTTP）、通信を暗号化する証明書（TLS 証明書）の残り日数、ログのエラー件数を 5 分ごとに点検し、結果を終了コード（0=正常 / 1=警告 / 2=危険 / 3=判定不能）で返す | 定期的に測る体温計 |

3 本は手で動かし続けるのではなく、Linux は systemd timer、Windows はタスクスケジューラという OS 標準の定時実行の仕組みに登録するところまでが本演習の範囲です（登録そのものの実施状況は、上の実施ステータスのとおりです）。

演習に使うホストは 2 台です。`lab-base01` は Linux（Ubuntu Server 24.04 LTS）の練習用 VM、`LAB-WINOPS1` は本演習のために新しく用意する Windows Server 2022 評価版の使い捨て VM です。

各ツールの実装手順は [4 章 構築手順書](#4-構築手順書)、試験項目は [5 章 試験項目書](#5-試験項目書)にあります。
...
- Windows ラボのネットワークは「ラボの中だけで閉じる」ように作る。[01 学習環境](./01-environment.md)の
  ホストオンリーネットワーク運用を引き継いだうえで、ラボでの操作ミスや不具合がホスト PC の外（家庭内 LAN・
  業務端末など）へ広がらないようにし、あわせて Windows ラボと Linux ラボを互いに切り離しておくことが目的である
  - 新規に用意する Windows ラボ VM は、**Host-only（ホスト PC と VM の間だけで通信する方式）または
    Internal（VM 同士だけで通信する方式）のスイッチだけに接続する**。外から VM へ入ってくる通信（inbound）、
    ホスト PC のポートに来た通信を VM へ転送する設定（port forwarding）、VM を家庭内 LAN へ直接つなぐ方式
    （bridge 接続）は、いずれも無効にする
  - Windows ラボは、`lab-base01` が使う `192.168.56.0/24` と**同一セグメントに置かない**（別の IP アドレス帯に
    置き、ネットワーク上で互いに見えない状態にする）。[Windows / AD 公開再現ラボ §3](../evidence/templates/windows-ad-lab.md#3-公開前の安全条件)の隔離原則を踏襲する
  - 例外は 1 つだけで、[5.4 章の TCK-05](#54-checkpylinux--windows-共通)（TLS 証明書チェックの正常系）は外部の
    実サイトへ疎通できないと試験にならない。この試験のときだけ [Windows / AD 公開再現ラボと同じ運用](../evidence/templates/windows-ad-lab.md#3-公開前の安全条件)で
    NAT（VM から外向きの通信だけを通す方式）を一時追加し、確認後に外す
...
初めて読むときは、この抜粋を上から順に全部追う必要はない。次の 3 か所だけ押さえれば、このスクリプトが何をしているかは説明できる。

1. `main()`（コード末尾）… 多重起動を防ぐロックを取る → 設定を読む → ディスク・サービス・ログを点検する → 掃除する → 終了コードを返す、という全体の流れ。まずここを読む
2. `cleanup()` … 既定（`apply=False`）ではログに「削除予定」と書くだけで、`--apply` を付けたときだけ `unlink()` に進む。二段階運用の実体はこの関数
3. `_is_whitelisted()` … 削除の直前に、対象パスがホワイトリスト配下かどうかを `Path.resolve()` で正規化してから確認する。シンボリックリンクや相対パス表記でホワイトリストの外へ出るのを防いでいるのがここ（[固有の注意点](#固有の注意点)参照）

残りは読み方が繰り返しになる。`check_disk_usage()` と `check_services()` は「値を取る → しきい値や状態と比べる → 辞書のリストで返す」という同じ形なので、片方を読めばもう片方も読める。`recent_errors()` は `journalctl` の出力行をそのまま返すだけである。
...
- **現在の状態**: **lab-base01・LAB-WINOPS1 という実機（VM）では、まだ 1 項目も実行していない。** 実行したのは、AI 支援セッションの作業環境上で動かした `routine.py`・`backup.py`・`check.py` の Linux 側コードだけで、その結果は[5 章 試験項目書](#5-試験項目書)の実測結果欄へ反映済み（実行環境の詳細は[付録：この作業環境での実行記録](#付録この作業環境での実行記録)）。内訳は次のとおり。

  | 区分 | 件数 |
  | --- | --- |
  | 試験項目 全体 | 49 |
  | この作業環境で実行した | 31（うち 30 項目が OK 判定） |
  | 未実施 | 18（Windows 側 `TW-` 全 11 項目、systemd timer / タスクスケジューラによる定期実行そのもの、ほか） |

  実行した 31 項目のうち 3 項目は、環境の制約に合わせて手順を調整している（`journalctl` のバグを発見・修正したうえで OK、権限テストは root では再現しないため非 root ユーザーを作って再実行、RTO 計測は稼働中の systemd timer が無いため手動起動で計測）。OK 判定にならなかった残り 1 項目（TCK-05）は、コード自体は正しく動作したが、外部の実証明書の残日数が実行時点でたまたましきい値を下回っていたため、想定した `OK` ではなく `WARNING` になった。実行を通じて `routine.py` の `journalctl` 呼び出しに実バグを 1 件発見し修正済み。
- **実施キット**: 同日、Windows 側・lab-base01 / LAB-WINOPS1 実機での実施を楽にするための実施キット（配置用スクリプト・
...
> （[server-monitor B-2〜B-4 演習](../../README.md#主な実測結果)・
