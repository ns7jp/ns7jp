...
**この演習を一言でいうと**: インターネットに公開した VPS（月額課金で借りる仮想専用サーバー）1 台に Ubuntu を入れ、
SSH 鍵ログイン・ファイアウォール・実ドメイン・無料の TLS 証明書まで設定し、再起動しても設定が保たれること、
そして 24 時間後・72 時間後も動き続けていることを確かめる演習である。本書はその設計であり、まだ実施していない。
...
順位 1・2 が採録先として使える「独立ホストの構築・試験手順」になる。

ただし、**本演習を完走しても順位 1・2 が完了扱いになるわけではない**。順位 1 の `site.yml`（Ansible）適用と
順位 3 の Slack 実配信は、本演習とは別に実施する必要がある。順位 2 についても、本演習が確認するのは
「再起動後にサービスが自動復帰すること」と「heartbeat が 24 / 72 時間後も届いていること」の 2 点までであり、
監視スタック本体の復帰確認とバックアップの復元試験は対象外のまま残る。
役割分担は[付録 B](#付録-b-本演習と既存資料の役割分担)にまとめている。
...
**HTTP-01 チャレンジとは**: Let's Encrypt が「申請者は本当にそのドメインの持ち主か」を確かめる方式のひとつ。
証明書を申請すると、Let's Encrypt 側のサーバーが `http://<申請したドメイン>/.well-known/acme-challenge/<ランダムな文字列>`
へ **80 番ポート**でアクセスし、`certbot` が一時的に用意した合言葉を読み取れれば持ち主だと認める。
つまり、取得するのは HTTPS（`443/tcp`）用の証明書だが、**この方式を使う限り、80 番ポートが外から到達できないと
取得も更新もできない**（[3 章](#ネットワーク)が `80/tcp` を開ける理由）。[5 章](#5-試験項目書)の T-14 は、この仕組みを
逆手にとって 80 番を閉じ、更新が失敗することを意図的に確かめる試験である。
