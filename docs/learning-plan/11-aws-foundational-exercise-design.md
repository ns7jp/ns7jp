...
| 上記と同一構成の Terraform 化、`plan`／`apply`／`destroy` の実行、2 回目 `apply` での冪等性（同じ操作を何度実行しても結果が変わらない性質）の確認 | **対象**。[5 章](#5-terraform化後半適用と削除まで実行) |
...
| EC2 メタデータ | IMDSv2 必須（`http_tokens = required`） | IMDS（Instance Metadata Service）は、インスタンス自身の情報（付与した IAM ロールの一時的な認証情報を含む）を `169.254.169.254` から取得する仕組み。旧来の v1 は HTTP の GET 1 回で読めるため、アプリ側の SSRF（Server Side Request Forgery: サーバーに任意の URL へ代理アクセスさせる攻撃）を踏み台に、その認証情報を読み出される経路になり得た。v2 は「先にトークンを発行してから読む」2 段階方式のため、GET 1 回だけでは読めない。[server-monitor 改善設計 03](../server-monitor-improvements/03-terraform-aws.md#5-モジュール設計)と同じ設定に揃える |
...
- **state（ステート）**: Terraform が「自分が作ったリソースはこれです」と記録しておく台帳ファイル（`terraform.tfstate`）。これがあるので、2 回目以降は「コードに書いた状態」と「台帳が示す現状」を突き合わせ、差分だけを直せる。state には接続情報などの機微な値が平文で入ることがあり、実行のたびに書き換わるため、Git には入れない（B-3 で `.gitignore` に追加する）。
- **冪等性（べきとうせい）**: 同じコードを 1 回実行しても 2 回実行しても、出来上がる状態が同じになる性質。手作業の手順書を 2 回なぞると EC2 が 2 台できてしまうが、Terraform は差分だけを埋めるため、コードと実物が一致していれば 2 回目の `apply` は「変更なし」で終わる。B-9 の `No changes` は「何も起きなかった＝失敗」ではなく「差分ゼロ＝合格」を意味する。
...
- [ ] IMDSv2 を必須にする理由（v1 は GET 1 回でメタデータを読めるため、SSRF を踏み台に IAM ロールの一時的な認証情報を読み出される経路になり得たこと）を説明できる
