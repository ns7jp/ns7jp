...
最終更新: 2026-08-28（学習設計の不足点セルフレビューを追加）
...
| P1 | **採点基準と提出例** | 「理解した」「説明できる」だけでは、人によって合否が変わる | 各 Phase に必須観点、合格条件、よい例、再提出になる例を用意し、別の評価者でも同じ判定になるか確認する |
| P1 | **開始前診断と迂回路** | PC 性能、利用 OS、既有知識、確保時間が違うと、同じ 24 週プランでは脱落しやすい | 10〜15 分の診断から「標準」「基礎補習」「低スペック環境」のいずれかへ案内し、飛ばしてよい条件を機械的に判断できる |
| P2 | **保守期限とフィードバック窓口** | コマンド、OS、クラウド画面は変わり、正しかった教材も古くなる | 対象バージョン、最終確認日、次回確認日、報告方法を各実施キットに記載し、四半期ごとにリンクと主要手順を再確認する |
