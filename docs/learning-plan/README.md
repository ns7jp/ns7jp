...
> - 本書は**学習計画**であり、server-monitor の改善設計ではありません。そのため、本リポジトリの「[新規設計を増やさない運用ルール](../evidence-capture-checklist.md#新規設計を増やさない運用ルール)」（対象は改善設計 06 以降）には当たりません。学習でできた成果物は新しい設計書を作らず、既存の[証跡採録チェックリスト](../evidence-capture-checklist.md)（＝実機で確かめた証拠を残すための項目一覧）へ足していきます（[本ポートフォリオとの接続](#10-本ポートフォリオとの接続)）

最終更新: 2026-09-01（初心者向け開始前診断、最小演習、学習ルート、採点例、第三者テスト様式を追加）
...
同じ内容を別コースとして最初からやり直す必要はありません。まず[主作品の初回実習](https://github.com/ns7jp/server/blob/main/docs/beginner-learning-guide.md)を一つ終え、継続するときは下表の役割で使い分けます。
...
最初から 24 週分を覚える必要はありません。毎回、次の型を繰り返します。
...
| 知る | これは何のために必要か |
| 試す | 最小の構成で動かせるか |
| 確かめる | どの出力を見て成功と判断するか |
| 壊す | よくある故障を安全なラボで再現できるか |
| 戻す | 元の状態へ戻せるか |
| 残す | 他の人が同じ結果を再現できるか |

用語に慣れていない場合は、先に[やさしい用語・見方ガイド](../beginner-guide.md)を読んでください。
実際に始めるときは、[初心者向けスタートガイド](./00-start-here.md)の開始前診断を行い、標準・軽量・基礎補習のいずれかを選んでください。

### 最初の 7 日間

| 日 | やること | 完了の目印 |
| --- | --- | --- |
| 1 | 学習用 VM を 1 台用意する | OS 名と IP アドレスを記録できた |
| 2 | 一般ユーザーと `sudo` を確認する | `whoami` と `sudo -v` の結果を説明できた |
| 3 | SSH で接続する | 別のターミナルから鍵で接続できた |
| 4 | サービスを起動・停止する | `systemctl status` の変化を確認できた |
| 5 | ログを読む | `journalctl` から対象サービスの記録を見つけた |
| 6 | スナップショット後に設定を 1 つ壊して戻す | 復旧前後の結果を保存できた |
| 7 | 1 週間を振り返る | 目的・結果・失敗・次の課題を 1 ページにした |

危険な操作は本番環境で行わず、削除可能なラボだけで実施します。コマンドを実行する前に、**対象・期待結果・戻し方**の 3 点を確認してください。

---

## 目次

読む順番は、まず [00 初心者向けスタートガイド](./00-start-here.md) → 次に [01 学習環境の作り方](./01-environment.md) → そのあと [02 フェーズ別カリキュラム](./02-curriculum.md) です。05 以降はテーマ別の演習設計なので、そのテーマを学ぶ週に開けば十分です。とくに 08〜13（Windows / 監視 / クラウド / VPS）は、必須範囲（Phase 1〜4 と Ansible / Git）を終えてから応募先に合わせて選びます（[00 の該当章](./00-start-here.md#3-24-週で必ずやること応募先に合わせて選ぶこと)）。

| ページ | 一言でいうと | 区分 |
| --- | --- | --- |
| 本ページ | 24 週で何をどの順に学ぶかの全体計画と到達度チェック | 必須 |
| [00 初心者向けスタートガイド](./00-start-here.md) | 最初の 30 分にやること、自分に合うルートの選び方 | 必須（最初に読む） |
| [01 学習環境の作り方](./01-environment.md) | 練習用の仮想マシンを自分の PC に用意する手順 | 必須 |
...
| [Windows コマンド集](../windows-commands.md) | コマンドプロンプトと PowerShell の基本コマンドを、目的から逆引きできる形でまとめた初心者向けリファレンス | 参考 |
...
| [08 AD構築演習設計](./08-ad-exercise-design.md) | Windows の利用者・権限管理（Active Directory）を、グループ設計からバックアップ・復元まで一通り作る演習（[Windows / AD 公開再現ラボ](../evidence/templates/windows-ad-lab.md)の先を扱う）。実施キット（[ad-exercise-kit/](./ad-exercise-kit/README.md)、構文検証済みの `.ps1` 全 11 本）あり。**設計のみ・未実施** | 演習設計 |
| [09 Zabbix 監視基盤構築演習設計](./09-zabbix-monitoring-exercise-design.md) | 監視ツール Zabbix でサーバーを監視する演習。本ラボの主系統である Prometheus / Grafana とは別の補完。**設計のみ・未実施** | 演習設計 |
| [10 Azure構築演習設計](./10-azure-foundational-exercise-design.md) | Azure でクラウド環境を作る演習。主系統の AWS + Terraform（[ADR-0005](../adr/0005-terraform-for-iac.md)）とは別の補完。**設計のみ・未実施** | 演習設計 |
| [11 AWS基礎構築演習設計](./11-aws-foundational-exercise-design.md) | AWS で最小構成（ネットワーク 1 つ + サーバー 1 台 = VPC + EC2）を手作業で作り、そのあと Terraform で作り直す演習。[STATUS.md の「Terraform 約 3,000 行が `apply` 0 回」](../../STATUS.md#コードでは埋められない残っている穴)を破る最初の一歩で、[server-monitor 改善設計 03](../server-monitor-improvements/03-terraform-aws.md)の本番想定設計とは別物。**設計のみ・未実施** | 演習設計 |
| [12 Windows Server 基礎構築演習設計](./12-windows-server-exercise-design.md) | 空の仮想マシンに Windows Server を入れ、名前・固定 IP・遠隔操作（WinRM / RDP）・更新までを設定する演習（`LAB-WINBASE01`）。**設計のみ・未実施** | 演習設計 |
| [13 恒久ホスト構築演習設計](./13-persistent-host-exercise-design.md) | 借りたサーバー（VPS）を「消さずに動かし続ける 1 台」にする演習。実ドメインの DNS 設定・通信の暗号化（TLS）・再起動試験・解約まで扱う。[STATUS.md の「恒久ホストが 1 台も無い」](../../STATUS.md#コードでは埋められない残っている穴)への対応。**設計のみ・未実施** | 演習設計 |
| [不足点セルフレビュー](#11-学習設計の不足点セルフレビュー) | この学習計画に足りていない検証と、その優先順位 | 参考 |
| [第三者再現テスト記録](./beginner-usability-test.md) | 初学者に試してもらった結果を記録する様式 | 参考 |
...
G3 の「L1 から L7」は OSI 参照モデルの階層を指します。用語は[やさしい用語・見方ガイド](../beginner-guide.md)を参照してください。
...
次の 9 語は、下の自己採点表と [10. 本ポートフォリオとの接続](#10-本ポートフォリオとの接続)の現在地表で使います。ここに無い語は[やさしい用語・見方ガイド](../beginner-guide.md)にあります。

| 言葉 | 意味 |
| --- | --- |
| 証跡（しょうせき） | 「やった」と言えるだけの記録。実行した日時・対象・コマンド・出力そのもの |
| `NOT RUN` | まだ実行していない、という状態表示。「失敗した」ではなく「未実施」 |
| PASS | 試験項目に合格したこと。「9 PASS」は 9 項目合格、「23/23 PASS」は 23 項目すべて合格 |
| `changed=0` | Ansible を 2 回目に実行したとき「変更なし」を示す出力。同じ手順を何度実行しても結果が変わらないこと（冪等性）の確認に使う |
| RTO | 本来は「何分以内に戻すか」という復旧時間の目標値。本書では復旧演習で実際に戻るまでにかかった時間（実測値）を指す |
| WSL2 | Windows の中で Linux を動かす仕組み。自分の PC の中だけで完結する環境 |
| 使い捨て runner（ランナー） | GitHub Actions が試験のたびに用意し、終わると消える一時的なサーバー。試験後は残らない |
| E2E（イーツーイー） | 端から端まで、部品単位ではなく全体を通して行う試験 |
| Molecule（モレキュール） | Ansible の設定のまとまり（ロール）を、使い捨てコンテナで自動試験する道具 |
...
経験ではなく**時間が足りない**ために短くしたい場合は、この表ではなく[時間が取れない場合の縮退版（12 週）](#時間が取れない場合の縮退版12-週)を使います。表に Phase 4 と Phase 6 が無いのは書き漏れではなく、この 2 つには短縮条件を設けていないためです（Phase 4 は縮退版でも削りません）。
...
結論として、**学ぶ範囲、演習手順、成果物の型（教材そのもの）は十分に揃っています。** 足りないのは、**この教材だけで初心者が迷わず最後までやり切れるかを、実際に試して確かめた記録**です。あわせて、別の人が採点しても同じ合否になるかも確認できていません。文書の量をさらに増やすより、次の順で実際の利用結果を集める方が価値があります。

### すでに揃っているもの

- Linux → ネットワーク → ミドルウェア → 文書化 → 自動化 → 監視という学習順序
- 各週の目的、ハンズオン、到達確認、つまずきやすい点
- 安全上の注意、切り戻し、秘密情報のマスキング、クラウドの課金対策
- パラメータシート、手順書、試験項目書、障害・復旧記録という成果物の定義
- 「設計のみ」「実測済み」「未実施」を混同しない状態表示

### 優先して補う 5 点

開始前診断、最初の30分、必須／選択ルート、各Phaseの採点基準と提出例、質問・保守方法、第三者テスト様式は[初心者向けスタートガイド](./00-start-here.md)として追加しました。ただし、**様式を作ったことと実地確認を完了したことは別です**。以下は実施結果が揃うまで未完了として扱います。

| 優先 | 不足しているもの | なぜ必要か | 完了条件 |
| --- | --- | --- | --- |
| P0 | **本人による基礎工程の通し実行** | 詳細な設計があっても、空の VM への OS 導入から引き渡しまでの実測がなければ、構築力の証明にならない | 初期状態、実行者、所要時間、失敗、再試験を含む証跡を公開し、試験項目をすべて実測値で埋める |
| P0 | **第三者による再現テスト** | 作成者には通じる省略が、初心者には最大の障害になる | 初学者 2 名以上が README だけを入口に演習し、質問箇所、停止箇所、完了率、所要時間を記録する。指摘を反映して再テストする |
| P1 | **採点基準の第三者確認** | 各Phaseの基準と共通提出例は追加したが、別の評価者が同じ合否を出すか未確認 | 2名以上の評価者が同じ提出物を採点し、判定が割れた観点を修正して再採点する |
| P1 | **開始前診断と迂回路の実地確認** | 標準・軽量・基礎補習の分岐は追加したが、異なるPC条件で妥当性を確認していない | 各ルートを対象条件の端末で実行し、飛ばしてよい条件と必要資源を実測値で更新する |
| P2 | **保守期限とフィードバック運用** | 入口には確認日と報告方法を追加したが、全実施キットへの展開と実運用が残る | 対象バージョン、最終確認日、次回確認日、報告方法を各実施キットに記載し、四半期ごとにリンクと主要手順を再確認する |

### 第三者再現テストで記録する項目

レビューを「分かりやすかったです」という感想だけで終わらせず、次を同じ形式で記録します。個人名や端末固有情報は公開しません。
...
教材を増やし続けないため、まず **P0 の 2 項目を最優先**にします。第三者テストで同じ箇所に 2 人が詰まった場合は教材側の欠陥として修正し、1 人だけが詰まった場合も前提知識または環境差を確認します。P0 が終わるまでは、新しい技術テーマを追加して「網羅性」を広げません。

このレビュー自体も設計であり、実績ではありません。実施結果が追加されるまでは、ポートフォリオ上で「初心者が再現できることを検証済み」とは表現しません。
