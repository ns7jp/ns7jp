...
| グループの種類とスコープ | 種類はセキュリティ／配布の 2 つ、スコープはドメインローカル／グローバル／ユニバーサルの 3 つ。組み合わせ方に定石（AGDLP）がある |
| FSMO | ディレクトリ全体の整合性に関わる 5 つの役割（フォレスト全体で 2 つ、ドメインごとに 3 つ）。単一 DC ラボでは全役割が 1 台に集中する |
...
| T-15 | 結合 | AGDLP 経由の権限付与 | 共有フォルダを 1 つ作成し `pf-dl-helpdesk-share-ro` に読み取り権限を付与済み | `pf-user01`（`pf-g-helpdesk-staff` 経由でネスト済み）でその共有フォルダを開く | 読み取りアクセスできる | | | | |
