...
>
> 本ドキュメントは主に**設計**です。3.3 演習 A（`backup-rotate.sh`）のみ、AI 支援セッションの作業環境で実行し試験項目書に採録済みですが、これは**本人が実機で再現・検証した記録ではありません**。他の演習は未実施です。詳細は [8. 実施ステータス](#8-実施ステータスと次のアクション)を参照してください。
...
- **現在の状態**: 3.3 演習 A（`backup-rotate.sh`）のみ、AI 支援セッションの作業環境で実行し 12/12 OK（[試験項目書](#試験項目書)、2026-08-26）。Level 1・2、演習 B、4 章（Windows／PowerShell）はいずれも未実施。**本人が実機（`lab-base01` 等）で再現・検証した記録ではない**点、[7 章](#7-証跡採録計画)が想定する `server-monitor` 側への一次証跡保存がまだの点は、上記実施記録の注記のとおり
- **次のアクション**:
  1. Bash 側（3 章）の残り（Level 1・Level 2・演習 B）に着手する。[1 章の前提条件](#前提条件)の Linux VM（`lab-base01`）が整い次第、本人の実機で演習 A も含めて再現し、AI 実行との差分があれば記録する
