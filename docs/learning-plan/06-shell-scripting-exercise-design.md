...
> **実施記録（2026-08-26）**: AI 支援セッションの作業環境（コンテナ、`Linux 6.18.44-fc-v21`、bash 5.2.21、root 権限）で本演習を実装・実行し、下表の 12 項目すべてを採録しました（12/12 OK）。**本人がこの結果を実機で再現・検証した記録ではありません**（[README の AI の利用について](../../README.md#3-つの前提ai-の利用を含む)と同じ区別）。想定していた Ubuntu 24.04 LTS の `lab-base01` とは別環境のため、T-07・T-09・T-11 は環境差分に応じた代替手順で確認しています（各行末の注記を参照）。raw ログはこのセッションの一時領域にのみ存在し、[7 章の証跡採録計画](#7-証跡採録計画)が想定する `server-monitor` 側への保存はまだ行っていません（エビデンス列は「セッション内実行記録」と表記）。
...
- [Windows コマンド集（初心者向け）](../windows-commands.md)
