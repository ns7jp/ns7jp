# 08 Zabbix 監視基盤構築演習設計：基礎からのオンプレミス監視構築
...
| # | 学習項目 | ハンズオン | 到達確認 | つまずきやすい点 |
| --- | --- | --- | --- | --- |
| M5-1 | ファイルシステムの自動検出 | `vfs.fs.discovery` の Discovery rule を確認し、アイテムプロトタイプ（`vfs.fs.size[{#FSNAME},pfree]` 等）がマウントポイントごとに実体化される様子を Data collection → Hosts → Discovery で見る | 新しいマウントポイントを追加すると、手動設定なしで新しいアイテム・トリガーが生成されることを確認する | 生成には Discovery rule の実行間隔（既定は数時間単位）がかかるため、即座には反映されない |
