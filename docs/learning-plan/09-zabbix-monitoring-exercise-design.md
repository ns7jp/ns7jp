...
| 障害注入と検知・通知・復旧の一連の演習（RTO 相当の計測。RTO＝目標復旧時間、障害発生から復旧までにかけてよい時間の目標値） | **対象**。[6 章](#6-障害演習検知から復旧までz-1) |
...
**この章で扱う 5 つの言葉**（Zabbix はこの 5 つを組み合わせて監視を作ります）。

| 言葉 | 一言でいうと | 例 |
| --- | --- | --- |
| Item（アイテム） | 集める値そのもの。「何を測るか」 | CPU 使用率、`agent.ping` |
| Trigger（トリガー） | 集めた値が異常かどうかを判定する式。「いくつを超えたら異常か」 | 空き容量が 10% 未満なら異常 |
| Action（アクション） | 異常と判定されたあと何をするか。「誰にどう知らせるか」 | Admin へメッセージを送る |
| Template（テンプレート） | Item や Trigger をひとまとめにした雛形。ホストにリンクして使う | Linux by Zabbix agent active |
| Low-Level Discovery（LLD） | 監視対象を自動で見つけて Item を自動生成する仕組み。章タイトルの Discovery はこれを指す | マウントポイントを自動検出 |

5.1 以降は「既製のテンプレートで値を集める（5.1）→ 自分で値を追加する（5.2）→ 異常を判定する（5.3）→ 知らせる（5.4）→ 自動で増やす（5.5）」という 1 本の流れで進みます。
