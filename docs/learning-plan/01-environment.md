...
> まずLinuxの基本操作を練習するVMを1台用意し、必要になった段階で増やします。
> **[開始前診断](./00-start-here.md)で選んだ標準・軽量ルートに合わせて**進めます。所要時間の目安は1日から2日です。

最終更新: 2026-09-11（軽量ルートと準備後の戻り先を整理。本人の環境構築完了を示す日付ではありません）
...
| --- | --- | --- | --- | --- |
| 軽量：基本操作から | 4コア | 8GB以上 | 30GB以上 | まずVM 1台（メモリ2GB・仮想ディスク20GB）。3台構成や監視全体の要件ではない |
| 標準：3台構成の候補 | 4コア以上 | 16GB以上 | 60GB以上 | 最初は1台。複製前に実際の空き容量とメモリを再確認する |
| 余裕を持つ目安 | 4コア以上 | 16GB以上 | 150GB以上 | OSイメージ、更新、スナップショットの保存にも余裕を取る |

- **メモリ8〜15GB、または空き30〜59GBなら軽量ルート**でLinuxの基本操作から始めます。下の3台構成はこの段階では作りません
- これらは教材の構成目安です。ホストOSの使用量や実際の空き容量も確認します。主作品の2サービス実習へ進む前は、[そのガイドの前提条件](https://github.com/ns7jp/server/blob/main/docs/beginner-learning-guide.md)を別に満たしてください
...
| WSL2 | Windows | Windows の中で Linux のコマンド環境を動かす仕組み（Windows Subsystem for Linux 2）。Linux コマンドの練習に最速 | **本物の OS 起動・ネットワーク構成の学習には不向き** |
...
- **Apple Silicon Macの場合はUTMまたはMultipass**を候補にします。教材と操作が合わなければ、症状と環境を[質問テンプレート](./00-start-here.md#5-30-分以上止まったとき)に記録します。クラウドは後の選択演習で、環境準備の代替として自動的に切り替えません
- **WSL2 だけで済ませない**でください。コマンド練習には最適ですが、本プランで扱う NIC（ネットワークアダプタ。サーバーをネットワークにつなぐ部品）の設定・ファイアウォール・複数台間の疎通・OS 再起動の挙動が学べません
...
3 層構成とは、役割ごとにサーバーを 3 台に分ける形のことです。利用者からの通信を最初に受ける **Web**（入口）、
処理を実行する **AP**（アプリケーション）、データを保存する **DB**（データベース）の 3 つに分けます。
図や表では Web / AP / DB と略して書きます。

1 台にまとめても動きますが、3 台に分けると「どの役割のところで止まっているのか」を切り分ける練習ができ、
実際の案件で多く使われている形をそのまま再現できます。

**最初に 1 台だけ作り、Phase 3 で複製して 3 台に増やします。**

これは標準ルートの構成です。軽量ルートでは1台を保ち、Phase 3に着いたら[1台VMでの3層コンテナ演習](../../archive/docs/server-engineer/stages/03-services.md)の前提を確認します。3台のVMを使う演習と、1台内のコンテナ演習は、実施した環境を区別して記録します。
...
VM への接続にはホスト PC 側の SSH クライアントを使います。追加インストールが必要かは OS によって異なります。

| ホスト PC の OS | 入手方法 |
| --- | --- |
| Windows | OpenSSH クライアントが標準搭載されており、PowerShell や Windows Terminal からそのまま `ssh` コマンドが使える。無効になっている場合は、設定アプリの「アプリ」→「オプション機能」から追加できる |
| Mac | ターミナルアプリに `ssh` が標準搭載されている |
...
手順4でOSを入れて一般ユーザーでログインできた時点で、手順5へ進む前に[最初の30分：状態を確認して記録する](./00-start-here.md#2-最初の-30-分状態を確認して記録する)へ戻り、設定変更前の状態を確かめます。主作品の実習から来た場合は、[初回実習の前提確認](https://github.com/ns7jp/server/blob/main/docs/beginner-learning-guide.md)へ戻ります。このページの基本操作用VMと、主作品の実習に必要なメモリ・Docker等の条件は別に確認してください。
...
| # | 対策 | 具体的な操作 |
| --- | --- | --- |
| 1 | ルートユーザー（アカウント作成時にできる、すべての操作ができる最上位のアカウント）に MFA を設定する | MFA は多要素認証のこと。パスワードに加えて、スマホアプリに表示される確認コードなどでもう一度本人確認する仕組み。アカウント作成直後に実施する |
| 2 | 作業用の IAM ユーザー（用途ごとに、できる操作を絞って作る利用者アカウント）を作り、以後ルートを使わない | 権限は必要最小限から始める |
...
> **注意**: 予算アラート（表の 3）の多くは、しきい値を超えたことを通知するだけで、自動的に課金を止めるものではない。
> 通知に気付かなければ課金は止まらず増え続けるため、**優先すべき対策は表の 5「終わったら必ず削除する」の削除確認**であり、
> 予算アラートはそれを補う保険と位置付ける。
...
上の表の「学習範囲」のうち、次の 3 つは別ドキュメントに演習として設計しています。

| 学習範囲 | 具体的にやること | 対応する演習設計 |
| --- | --- | --- |
| OS の初期構築 | 空の VM への導入、ホスト名、固定 IP、ローカル管理者、リモート管理（WinRM / RDP＝別の PC から操作するための入口）、ファイアウォール、Windows Update、サービスとタスクの基礎 | [12 Windows Server 基礎構築演習設計](./12-windows-server-exercise-design.md) |
| PowerShell での一括操作 | 基礎文法から、サービス操作、イベントログ確認、AD 操作まで | [06 シェルスクリプト演習設計](./06-shell-scripting-exercise-design.md) |
| OU 設計とグループポリシーの基本 | 利用者やパソコンを整理してまとめる入れ物（OU）の設計と、設定を一括で配る仕組み（グループポリシー）の基本 | [08 AD構築演習設計](./08-ad-exercise-design.md) |
