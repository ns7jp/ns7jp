...
（[README](../../README.md#追加の実測演習)に実行環境を明記）。
...
- **現在の状態**: 設計のみ。3〜7 章・[付録 A](#付録-a-hyper-v-版の差分)のいずれも実機（VM）で実行していない。
  ...
  同日、実施を楽にするための実施キット（ネットワーク設定ファイル・sshd 設定ファイルの雛形、Hyper-V 用
  PowerShell スクリプト、進捗チェックリスト、証跡記入用テンプレート）を [phase1-kit/](./phase1-kit/README.md) に
  用意した。これも AI 支援セッションでの作成のみで未実行であり、上記の実施ステータスを変えるものではない
...
>（[B-2〜B-4 演習](../../README.md#追加の実測演習)と同じ `6.18.44-fc-v21`
...
- [Phase 1 演習 実施キット（Hyper-V 向け、未使用の雛形）](./phase1-kit/README.md)
- [学習プラン 全体像](./README.md)
