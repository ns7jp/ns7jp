...
**一言で言うと**：自分の PC の中に空の仮想マシン（VM）を 1 台つくり、そこに Linux（Ubuntu Server 24.04）を
インストールして、「決まったホスト名・決まった IP アドレスを持ち、SSH 鍵を持っている人だけがログインでき、
ファイアウォールで SSH 以外を通さないサーバー」に仕上げるまでの設計書です。設定値の一覧＝パラメータシート
（[3 章](#3-パラメータシート)）、作業手順＝構築手順書（[4 章](#4-構築手順書)）、出来ばえの確認項目＝試験項目書
（[5 章](#5-試験項目書)、全 21 件）の 3 点で構成しています。**この演習自体はまだ実施しておらず、試験項目書の
実測結果欄はすべて空欄です。**

**はじめて読む人へ**：まず [2 章の流れ図](#基本設計構成と選定理由)で「空の VM から何をどの順に設定するか」を
つかみ、次に [4 章](#4-構築手順書)を上から読むと迷いません。すぐ下の「本ドキュメントの位置付け」の引用
ブロックは、この設計書がどの資料から派生し、どこまで事前検証したかを記録した部分なので、初回は読み飛ばして
構いません。
...
[学習プランの自己採点](./README.md#本人の到達状況自己採点)（採点の正本）には、目標を G1・G2… と番号で並べ、
**○ = 根拠を出せる / △ = 一部のみ / × = 未着手**の 3 段階で自己採点した表がある。そのうち **G1**（空の VM から
**Web / AP / DB の 3 層構成**を単独で構築）は「△」である。AP はアプリケーションを動かすサーバーのことで、
Web・AP・DB の 3 つの役を別々のサーバーに分ける構成を 3 層構成と呼ぶ。G1 が △ である根拠欄には
「コンテナ上の構成であり、空の VM に OS を入れて組んだ実績ではない」と明記されている。これは
[STATUS.md](../../STATUS.md) の「コードでは埋められない、残っている穴」の 2 番目
...
B-4 が network namespace（1 台の Linux の中に仮想のネットワークだけを作る仕組み）であり、
**いずれも AI 支援セッションの作業環境上での実行**で、独立した物理／VPS ホストや
手元 WSL2（Windows の中で Linux を動かす仕組み）での再実行証跡ではない
（[README](../../README.md#主な実測結果)に実行環境を明記）。
いずれも「VirtualBox 等の一般的なハイパーバイザー（仮想マシンを作って動かすソフト）上で、
空の VM に手作業で OS を導入する」経路とは別物である。違いは仮想化ソフトの種類ではなく、
**どこで実行したか（AI 支援セッションの作業環境か、自分の手元か）と、OS を自分の手で入れたかどうか**にある。
**本演習は、この埋まっていない経路（空の VM に手作業で OS を入れる）を埋める。**
...
> **「ドロップイン」と読み込み順（この演習でいちばん間違えやすい点）**
>
> - **ドロップイン**とは、本体の設定ファイルを直接書き換えず、専用ディレクトリ
>   （`/etc/ssh/sshd_config.d/`、`/etc/netplan/`）に自分用のファイルを 1 枚置いて設定を足すやり方。
>   どちらのディレクトリも、ファイル名の**辞書順**（`00-` → `50-` → `60-` → `99-`）に読まれる。
> - 違うのは、同じ項目が 2 つのファイルに書かれていたとき、どちらが採用されるかである。向きが逆になる。
>   - **sshd は先勝ち**（先に読んだ値を採用する）。だから本演習の SSH 設定は `00-lab-hardening.conf`。
>   - **netplan は後勝ち**（後に読んだ値で上書きする）。だから本演習の固定 IP は `60-lab-static.yaml`。
> - どちらのディレクトリにも、Ubuntu インストーラ由来の `50-` で始まるファイルが先に置かれていることがある。
>   この向きを取り違えると、自分が書いた設定が黙って無視される。それぞれの詳細と確認手順は
>   [3-5 の注意](#3-5-固定-ip-の設定)と[3-8 の注意](#3-8-パスワード認証root-ログインの禁止)にある。
...
> **番号の読み方**：この章の中の「1. 概要」「2. 作業前確認」「3. 作業手順」は、
> [03 構築工程の実務ドキュメント](./03-build-process.md#3-構築手順書)が定める手順書の内部構成であり、
> 目次の章番号（3 章 = パラメータシート、4 章 = 構築手順書）とは別物である。
> 本文の `3-5` や `3-7-4` はこの章の「3. 作業手順」の中の手順番号で（`3-5` = 固定 IP の設定、
> `3-7-4` = 鍵ログインの動作確認）、同様に `2-1` は「2. 作業前確認」、`4-1` は「4. 作業後確認」の中の番号である。
> このほか、`T-` は[5 章 試験項目書](#5-試験項目書)の試験項目、
> `R-` は[切り戻し手順](#52-切り戻し手順作業手順と同じ粒度)、
> `P-` / `Q-` は試験用のネットワークを一時的に足して撤去する手順
> （[P-1〜P-7](#t-09--t-13-の前提検証用セグメントの一時追加) が VirtualBox 版、
> [Q-1〜Q-7](#a-3-検証用セグメントの一時追加p-1p-7-の代替) がその Hyper-V 版）、
> `A-` は[付録 A](#付録-a-hyper-v-版の差分)の節番号を指す。
...
> **3-8 の注意**: 結論から先に言う。**`/etc/ssh/sshd_config.d/` に先に置かれたファイル（`50-cloud-init.conf`）が
> ある場合、自分のファイル名をその番号より小さくしないと、ここで書いた `PasswordAuthentication no` は
> 黙って無視される。** 本演習が `00-lab-hardening.conf` という名前を使うのはこのためである。
>
> 理由は次のとおり。インストーラで SSH 鍵を取り込まなかった場合、Subiquity は初回起動時の cloud-init に
> `ssh_pwauth: true` を渡し、cloud-init が `/etc/ssh/sshd_config.d/50-cloud-init.conf` を作成して
> `PasswordAuthentication yes` を書く。一方 sshd は、本体 `sshd_config` の先頭近くにある
> `Include /etc/ssh/sshd_config.d/*.conf` の行によって、このディレクトリのファイルを**ファイル名の辞書順**に、
> かつ本体の他の設定より先に読み込み、同じ項目については**最初に読んだ値**を採用する。つまり自分のファイル名が
> `50-` より後（例: `99-`）だと、先に読まれた `50-cloud-init.conf` の `yes` が残り、こちらの `no` は負ける。
> `sshd -t` は構文チェックだけなので、この「負け」をエラーとして教えてくれない。
>
> だから本演習では `00-` を使って先に読ませ、3-8-5 の「実効値の確認」（`sudo sshd -T` で、sshd が最終的に採用した
> 値を一覧表示させる）で必ず裏を取る。
>
...
この 21 項目は「手順どおりに作業できたか」ではなく「**作ったサーバーが狙いどおりに動くか**」を確かめるためのもの。
分類は [03 §4 の試験分類](./03-build-process.md#試験分類)に従っており、本演習では次の 4 つに分かれる。

- **単体**（T-01〜T-06・T-19〜T-21 の 9 件）: 設定 1 つが効いているかを確かめる（例: ホスト名、固定 IP）
- **結合**（T-07〜T-09 の 3 件）: ホスト PC からサーバーへ、複数の設定をまたいだ動きを確かめる
  （例: パスワード認証の拒否、ファイアウォールで開けたポートと閉じたポート）
- **総合**（T-10・T-11 の 2 件）: 利用者の視点で一連の流れを確かめる（例: 再起動しても全設定が戻るか）
- **異常系**（T-12〜T-18 の 7 件）: **わざと失敗させて**、想定どおりに失敗すること（項目によっては
  そこから復旧できること）を確かめる（例: 登録していない鍵でのログイン、`/var` の容量枯渇）
...
| `netplan generate` / `netplan try`（[3-5](#3-5-固定-ip-の設定)）、`unattended-upgrade --dry-run`（[T-20](#5-試験項目書)） | **一言で言うと**: この作業環境の Python のバージョンがずれていて、`netplan` と `unattended-upgrade` を実際に走らせられなかった（netplan の設定ファイルは YAML の文法が正しいことまでしか確認できていない）。<br>詳細: このセッションの `/usr/bin/python3` が標準の 3.12 系ではなく 3.11 系に差し替わっており、OS 同梱パッケージが `cpython-312` 向けにビルドしたネイティブ拡張（netplan の `_cffi_backend`、`unattended-upgrade` が使う `apt_pkg`）を読み込めない（`/usr/bin/python3.12 -c "import apt_pkg"` は成功することを確認したため、Ubuntu 24.04 自体の欠陥ではなくこのセッション固有の Python 差し替えが原因と特定した）。YAML の構文チェックにとどめ、netplan 独自のスキーマ検証・実際のインターフェースへの適用・`unattended-upgrade` の dry-run 実行結果は未確認 |
