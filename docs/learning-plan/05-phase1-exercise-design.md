...
| NIC 枚数 | 2（[01 学習環境 §3](./01-environment.md#3-ラボ構成3-台構成)のネットワーク設計に準拠）。[T-09 / T-13](#5-試験項目書)の実施時のみ検証用の 3 枚目を一時的に追加し、試験後に撤去する |
...
| --- | --- | --- | --- |
| 2-1 | ホストオンリーネットワークの存在確認 | VirtualBox の「ホストネットワークマネージャー」を開く | `192.168.56.0/24` のアダプタが存在する |
...
| No | 作業内容 | 操作 | 想定結果 | 判定 |
| --- | --- | --- | --- | --- |
| 3-1 | VM 新規作成 | VirtualBox で新規 VM を作成。vCPU 2 / メモリ 2GB / ディスク 20GB、NIC1=NAT、NIC2=ホストオンリー（`192.168.56.0/24`）を設定 | VM 一覧に `lab-base01` が表示される | 設定値がパラメータシートと一致 |
| 3-2 | OS インストーラ起動 | ISO をマウントして起動 | Subiquity（Ubuntu Server インストーラ）の言語選択画面が表示される | 画面が表示される |
...
| 3-7-5 | スナップショット取得（SSH 強化の直前） | VirtualBox で `key-login-ok` という名前のスナップショットを取得 | 一覧に `key-login-ok` が表示される | 鍵ログイン成功（3-7-4）を確認した状態で取得済み |
...
| 3-11-5 | スナップショット取得 | VirtualBox で `base-clean` という名前のスナップショットを取得 | スナップショット一覧に表示される | [01 学習環境 §4 のスナップショット運用](./01-environment.md#スナップショット運用)に準拠 |
...
VirtualBox のコンソール（SSH 不要）から実施する。適用済みの手順の逆順に、必要な行だけを実行する。
...
> から `nc -zv 192.168.56.10 22` を実行して成功を確認する。
...
| 3:00 | 再起動試験・スナップショット（3-11、`base-clean` 取得） | 再起動後にすべての設定が保持されている |
...
| GUI 手順の証跡 | 3-1〜3-3 と 3-11-5 は端末ログに残らないため、スクリーンショットで採録する。最低限、VM 設定画面（vCPU 2 / メモリ 2GB / NIC 2 枚）、ストレージ選択画面、インストール完了画面、`base-clean` を含むスナップショット一覧の 4 枚 |
