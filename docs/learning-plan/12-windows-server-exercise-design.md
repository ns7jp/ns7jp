...
Windows Server の `LAB-WINBASE01` に対して行います。**一言でいうと「空の VM 1 台に Windows Server 2022 評価版を入れ、
他の演習が『もう済んでいる前提』にしている状態を、自分の手で作れるようにする」演習です。**

具体的には次の 4 つを、コマンドと想定結果まで書き下します。

1. VM を用意して OS を入れ、ホスト名・時刻（タイムゾーンと時刻同期）・固定 IP を決める（[4.1](#41-作業前確認)〜[4.5](#45-固定-ip-の設定)）
2. 管理者アカウントを整え、Hyper-V のコンソールを開かずにホスト PC 側から操作できるようにする（WinRM / RDP。[4.6](#46-ローカル管理者ユーザーと組み込みアカウントの整理)〜[4.8](#48-リモートデスクトップの有効化)）
3. ファイアウォールで入口を絞り、Windows Update を当て、ロールと機能の初期状態を記録する（[4.9](#49-ファイアウォール)〜[4.11](#411-ロール機能ベースラインの記録)）
4. サービス・イベントログ・定期実行という日常運用の入口に触り、再起動しても設定が残ることを確かめてチェックポイントを取る（[4.12](#412-サービス操作とイベントログの確認)〜[4.14](#414-再起動試験とチェックポイント取得)）
...
未経験者が本書の決定事項を読むには、最低限次の語彙が要る。[08](./08-ad-exercise-design.md#2-ad-の基礎概念と要件基本設計)が AD の語彙を先に示しているのと同じ位置付けで、より詳しい説明は[付録 A](#付録-a-windows-server-基礎用語辞典)に譲り、ここでは本章の決定事項を理解するために必要な範囲だけを先に示す。

| 用語 | 最小限の説明 |
| --- | --- |
| ワークグループ | ドメイン（AD）に参加していない状態。アカウントをその 1 台の中だけで管理する。`LAB-WINBASE01` は最後までこの状態のまま |
| Desktop Experience / Server Core | Windows Server の 2 つの導入形態。前者は GUI あり、後者は GUI なし。本書は前者を選ぶ |
| ネットワークカテゴリ（ネットワークプロファイル） | その接続を Windows が「ドメイン／プライベート／パブリック」のどれとみなすかの区分。区分ごとにファイアウォールの既定が変わる |
| WinRM / PowerShell リモーティング | Windows をリモート管理するための基盤。他の PC から `Enter-PSSession` などで PowerShell のコマンドを送り込むのに使う。本書が使うリスナーは HTTP（`5985/tcp`） |
| `TrustedHosts` | ワークグループではドメインの仕組みで相手を確認できないため、「この宛先は信頼する」と接続する側にあらかじめ登録しておくリスト |
| NLA（ネットワークレベル認証） | リモートデスクトップで、デスクトップ画面を出す前に認証を済ませる設定 |
| ベースライン | 「まだ何も足していない状態」を記録したもの。後から何が増えたかを比べるために取る |
...
| リモート管理方式 | **WinRM**（HTTP + `TrustedHosts`、ラボセグメント限定）を主、**RDP**（NLA 必須、ラボセグメント限定）を副として両方有効化する | **WinRM を有効化する理由**: PowerShell で他のホストを操作するときの標準の土台であり、複数ホストにまたがる一括管理・監視など将来の演習に備えた汎用インフラだから。加えて、正しく構成してアクセス元をラボセグメントに絞れること自体が Windows Server の基礎であり、本書でも[4.7](#47-リモート管理の有効化winrm)で有効化して[試験項目書](#5-試験項目書) T-13 でホスト PC からの接続を確認する。**他の演習の前提ではない**: 設計済みの [06](./06-shell-scripting-exercise-design.md)・[07](./07-python-ops-automation-exercise-design.md)・[08](./08-ad-exercise-design.md#46-gpo-のリンクとクライアント側の適用確認)はいずれも対象ホスト自身のローカルコンソール上で完結し、WinRM 経由のリモート実行を前提にしていない（07 は複数ホストをまたぐリモート一括実行を明示的に対象外としている）。**RDP を併設する理由**: [志望トラック](../target-roles.md)の補助トラック（IT サポート・社内 SE 補助）の実務で使用頻度が高いから。**HTTPS 化・証明書運用は対象外**: 基礎の範囲を超えるため、[付録 A](#付録-a-windows-server-基礎用語辞典)に将来課題として記す |
| ネットワークカテゴリ | 隔離 Internal スイッチの NIC を明示的に **Private** に設定する | 未識別のネットワークは既定で **Public** に分類される。Windows Server では、Public のままでも `Enable-PSRemoting` はファイアウォール規則を作るが、その規則は「同じサブネットからだけ許可」という制限つきになる。本ラボはホスト PC（`192.168.60.1`）と VM（`192.168.60.10`）が同じサブネットなので、この制限つき規則だけで**たまたま繋がってしまう場合がある**。たまたま繋がる状態に頼らず、意図を設定として残すため、[4.7](#47-リモート管理の有効化winrm)の最初にカテゴリを Private にする。これは[05 の netplan/cloud-init 順序](./05-phase1-exercise-design.md#3-5-固定-ip-の設定)と同種の「知らないと見落とす」落とし穴で、クライアント版 Windows との違いを含む詳しい挙動は[付録 A](#付録-a-windows-server-基礎用語辞典)にまとめる |
...
| タスクスケジューラ演習の対象 | 単純な日次ハートビート記録タスク 1 本の登録・実行確認に限定する | [06 演習 C-2 フラッグシップ `Invoke-EnvironmentCheck.ps1` の日次登録](./06-shell-scripting-exercise-design.md#演習-c-2フラッグシップ-invoke-environmentcheckps1)と重複させないため、本書はタスクスケジューラという機構そのものの基礎操作（アクション・トリガー・実行結果の確認）だけを扱う |
...
> **4.7-1 の注意**: この行を飛ばすと、WinRM を有効化しても繋がらないか、逆に「たまたま繋がっているだけ」の
> 状態になる。理由は[2 章の決定事項](#決定事項選定と理由)と[付録 A](#付録-a-windows-server-基礎用語辞典)を参照。
...
実務水準のタスク（環境チェック・失敗時通知等）は [06 演習 C-2 フラッグシップ](./06-shell-scripting-exercise-design.md#演習-c-2フラッグシップ-invoke-environmentcheckps1)が扱う。
