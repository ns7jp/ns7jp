...
> 2026-09-08更新：以下の設計を基に、最新GitHub収集・履歴管理・限定修正案生成を実装しました。
> 現在の実装範囲と毎週月曜09:00 JSTの運用は[週次運用手順](weekly-operation.md)を優先してください。
> 本文にある日次・土曜の時刻や未実装の機能は、初期の設計案として残しています。
