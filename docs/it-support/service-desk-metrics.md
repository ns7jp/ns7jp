...
| Sev | 例 | 応答 SLO | 解決 SLO |
| --- | --- | --- | --- |
| **Sev1** | 全社的にサービス停止、業務停止 | 気づき次第すぐに対応 | 4 時間以内 |
| **Sev2** | 部署単位の停止、業務に支障 | 当日中 | 1 営業日以内 |
| **Sev3** | 個人の業務に影響、回避策あり | 1 営業日以内 | 3 営業日以内 |
| **Sev4** | 個別質問、機能要望 | 数営業日以内 | 1 週間以内 |

各 Sev で、月にどれくらい目標時間内に対応できたかを振り返ります（例：「Sev3 は目標時間内に解決できたチケットが月 90%」）。

---

## 4. ダッシュボード設計

（以下は架空の記入例です。実際の対応件数・数値ではありません）
...
| --- | --- |
| 可用性 SLO | Sev 別解決 SLO |
| エラーバジェット（許容できる遅れの目安） | バックログ閾値 |
| バーンレートアラート（滞留の増加ペースへの警告） | 24h 滞留チケット警告 |
| 月次 SLO レビュー | 月次 Service Desk レビュー |
| ポストモーテム（[07](../roadmap/07-incident-response.md)） | CSAT 低スコアフォロー |
| 不在時にも他者が対応できるかの訓練（[17](../roadmap/17-chaos-engineering.md)） | 「IT 担当不在で他者が対応できるか」演習 |
