...
| 3 | IT サポート・社内 SE 補助 | 問い合わせ、キッティング、棚卸し | [AD 操作演習設計](./learning-plan/06-shell-scripting-exercise-design.md#44-level-4-active-directory-運用スクリプト)、[Windows / AD 公開再現ラボ](./evidence/templates/windows-ad-lab.md)（**設計サンプル。実務対応実績ではありません**） | 実機出力を添えた Windows / AD 切り分け記録 |
