...
このページで挙げる 9 月の Windows Server / AD の記録は、AI の支援（手順の案内など）を受けながら私が手元の Hyper-V 上の VM を操作し、結果を画面で確認したものです。
...
| IT サポート・社内 SE | [AD 操作演習設計](./learning-plan/06-shell-scripting-exercise-design.md#44-level-4-active-directory-運用スクリプト) | トライアル就業の研修で遭遇したドメイン参加の失敗（DNS の向き先の誤り）を例にした、確認順と記録方法 |
...
- B-1〜B-4 の演習は **AI 支援セッションの作業環境上での実行**で、独立した物理／VPS ホストや
  手元 WSL2 での再実行証跡ではありません（実行環境は[検証証跡台帳](https://github.com/ns7jp/server/blob/main/docs/evidence/README.md)に明記）
