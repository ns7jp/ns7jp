...
最終更新: 2026-08-22（**優先 1・2・3・5・6・7 と Full-stack E2E を採録完了**。残るのは Slack 実配信、Windows / AD、AWS、D-2、独立した管理端末・引き渡し対象ホスト、組織 DNS、長期稼働の確認）
...
>
> **2026-08-22 に Full-stack E2E も採録しました。** Docker 導入済みの disposable Ubuntu 24.04 runner で、`site.yml` の一括適用と 2 回目 `changed=0`、core 9 services + CI webhook sink、local webhook の FIRING / RESOLVED、network / UFW、D-1 RTO 1 秒、3 volumes の backup / restore を確認し、[23/23 ID PASS](https://github.com/ns7jp/server-monitor/blob/53e885742593c4f5d96b8a8038e234f9a69947e0/docs/evidence/2026-08-22-full-stack-e2e.md)でした。Slack 実配信、AWS、D-2、独立した管理端末・引き渡し対象ホスト、組織 DNS、長期稼働の証跡へは読み替えません。
...
> 優先 8 は、**トライアル就業先で Windows サーバー・AD の構築研修が既に始まっているため優先度を上げます**（実務と学習が同じ方向を向くため効率が良い。就業先は Windows / Linux 双方のサーバー構築と AWS / Azure の研修を行っている。詳細は [resume.md](./resume.md) の「現況」参照）。
...
> ここで対象にしている従来の baseline CI は、構文・設定の整合・依存の脆弱性を確認するもので、起動・疎通・復旧時間は含みません。起動から復旧までを扱う [Full-stack E2E](https://github.com/ns7jp/server-monitor/blob/53e885742593c4f5d96b8a8038e234f9a69947e0/docs/evidence/2026-08-22-full-stack-e2e.md)とは区別して台帳へ記録します。
...
> **進め方の判断**: Linux サーバー構築・運用トラック（優先 1〜7）を主軸にしつつ、優先 8 も進めて各応募先に対応する実行証跡を増やします。
