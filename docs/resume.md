...
| 在籍期間 | 勤務先 | 雇用形態 | 業務内容 |
| --- | --- | --- | --- |
| （記入）YYYY/MM - YYYY/MM | （記入）業種・規模（社名は伏せて可） | （記入） | （記入） |
| （記入）YYYY/MM - YYYY/MM | （記入）業種・規模（社名は伏せて可） | （記入） | 在庫管理、ピッキング、入出庫、現場の業務改善 |

> **記入時の注意**: 社名を伏せる場合も、**在籍年月・社数・雇用形態は必ず記入してください。**
> 日本の中途採用の一次スクリーニングは在籍年月・社数・空白期間を時系列で追うところから
> 始まるため、ここが空欄だと内容の良し悪し以前に判定できません。年月は離職票または
> 源泉徴収票で確定させてください。
...
| 期間 | 状況 |
| --- | --- |
| （記入）製造・物流を離職 - 2025/09 | （記入）求職活動 / 職業訓練の待機など |
...
> 職務経歴書（PDF）と履歴書は、個人情報を含むため公開リポジトリには置かず、応募時に別途提出します。
...
| 勤務地 | （記入）在住地・通勤可能圏を記入してください |
| 希望年収 | （記入）応相談とする場合もその旨を明記してください |
...
| OS | Linux サーバー構築・基本運用（Ubuntu） | ○（個人ラボと研修での構築・確認。実務としての本番運用は未経験） |
 | OS | RHEL 系（AlmaLinux / Rocky 9） | △（Ansible role を `dnf` / firewalld / SELinux / dnf-automatic 対応にし、Molecule `el9` シナリオを実装。**シナリオの実行証跡は未採録、実機ホストへの適用も未実施**） |
