...
| 勤務地 | 東京都内通勤可能圏 |
| 希望年収 | 応相談 |
| 雇用形態（客先常駐 / SES の可否） | **（記入待ち）** 日本の未経験可インフラ求人は客先常駐（SES）形態が多いため、許容する場合はその旨、避けたい理由がある場合はその理由を一行で記入してください |
| 英語力 | **（記入待ち）** TOEIC 等のスコアがあれば記載。無い場合も「AWS / Cisco / Red Hat の英語ドキュメント・エラーメッセージの読解は学習中」等、現状を正直に一行で書いてください |
| 普通自動車運転免許 | **（記入待ち）** 保有していれば取得時期を記載。現地保守・複数拠点巡回のある求人で歓迎条件になることがあります |
