...
| 性能 | 段階負荷の実行と、HTTP エラーを含めた結果評価 | [9/17 の既存 CI 再分析](https://github.com/ns7jp/server/blob/main/docs/evidence/2026-09-17-performance-ci-analysis.md)で、並列 4 / 8 の HTTP 502 が内部基準 1% を超過。旧 PASS は失敗率の集計漏れがあり、性能合格とは扱わない。本人環境と修正後の実環境再試験は **NOT RUN** |
...
**修正後の負荷再試験と本人環境での実施**、独立した引き渡し対象ホストの受け入れ、組織 DNS、ホスト再起動後の永続性、24 / 72 時間の稼働、別の新規 VM への復元、
