...
- **次の一手** は優先順です。1 番目から着手し、コマンド例があればそのまま使えます。
...
4. 許可された環境で全試行を測定し、証拠を `evidence/` に、結果を `results.json` に置きます。`data_kind` は実測だけ `measured`。
5. 次の `run` が判定し、`done/` へ移し、同じ実行で追試候補を出します。次の一手に `DECIDE` が出ます。
...
- ロックが残ったら `status` で pid の生死を確認し、動いていないと確認した当該ロックだけを除去します。ループは自動で外しません。
