...
- 採否台帳 `decisions.json` への記録 `done` と、動的候補の却下 `dismiss`。次回の作業枠判定に反映します。
...
- 合成データの実行は `DEMO` と表示し、実測記録・本人の実績へは転記しません。合成の結果を学習台帳へ入れる操作は拒否します。
- 探索の候補・仮説は提案です。実験の実施、採用、公開の許可ではありません。
- ネットワークは `run` の実行時に既存の収集器が行う GitHub API の GET だけです。プロキシ環境や API 制限では収集不全（終了コード 3）になり、依存する準備を止めます。
