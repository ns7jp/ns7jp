...
2. ~~ネットワーク切り分けの一次メモ（優先 6）~~ → ⚠ **2026-08-21 一部実施**。ポート公開の不具合を発見・原因特定し、名前解決は `docker compose exec` 経由で確認できたが、経路（traceroute）とパケット（tcpdump）は未実施（[記録](https://github.com/ns7jp/server-monitor/blob/main/docs/evidence/2026-08-21-network-firstlook.md)）
