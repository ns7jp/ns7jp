...
| 項目 | 内容 |
| --- | --- |
| 新規作成 | [IT 基礎用語集](./docs/it-glossary.md)（20 分野・約 300 語。各語を「読み方・英語／ひとことの意味／覚え方と注意」の形式で統一し、章ごとに「つまずきポイント」を付記） |
| 収録範囲 | コンピューターとサーバーの基本、OS と Linux、ファイルと権限、プロセスとサービス、ネットワーク、名前解決と Web 通信、Web システム構成、仮想化・コンテナ・クラウド、構築の自動化、監視とログ、セキュリティ、ストレージとバックアップ、障害対応と運用指標、バージョン管理とデプロイ、データベース、単位の読み方、略語早見表（78 語） |
| 記憶の補助 | まちがえやすい用語の対比 33 組（冗長化とバックアップ、RPO と RTO、`502` と `504`、`start` と `enable` など）、語源から覚える表 13 語、実物をコマンドで確認する対応表 14 項目、復習間隔の指針、30 問のセルフチェックと解答 |
