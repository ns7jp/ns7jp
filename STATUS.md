...
最終更新：2026-09-09（[12 Windows Server 基礎構築演習設計](#2026-08-27-の更新内容追補12-windows-server-基礎構築演習設計の技術的レビュー修正)・[13 恒久ホスト構築演習設計](#2026-08-28-の更新内容追補13-恒久ホスト構築演習設計の技術的レビュー修正)には merge 直後の独立レビューを実施していたが、[11 AWS基礎構築演習設計](./docs/learning-plan/11-aws-foundational-exercise-design.md)だけは未実施だった差分を埋め、内部一貫性レビューで実在する欠陥 2 件（`terraform plan` で追加されるリソース数の誤り「8 種類」→ 正しくは実コード数え上げで 10 個、試験項目書の異常系件数の誤り「5 件」→ 正しくは表を数えて 6 件）を修正した。設計のみ・未実施という実施ステータス自体は変わらない。詳細は下記「1. 本リポジトリ」の該当エントリを参照。以下は 2026-09-08 時点の更新内容。
